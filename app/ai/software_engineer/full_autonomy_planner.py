...
class FullAutonomyPlanner:
    ...
    def plan(
        ...
        *,
        ...
        scoped_project_intelligence = self._is_scoped_project_intelligence(
            plan_metadata
        )
        provided_campaigns = [
            dict(item)
        ...
        feature_intent = None
        if (
            not scoped_project_intelligence
            and not provided_campaigns
            and not targets
        ...
        elif scoped_project_intelligence:
            target_files = self._scoped_targets(
                targets,
                replacements=replacements,
            )
            prepared_campaigns = self._build_scoped_campaigns(
                normalized_objective,
                target_files,
                replacements=replacements,
            )
            planning_source = "project_intelligence_scoped"
        else:
        ...
        minimum_targets = 2 if scoped_project_intelligence else self.min_targets
        if len(target_files) < minimum_targets:
            raise ValueError(
                f"Pełna autonomia wymaga co najmniej {minimum_targets} "
        ...
        if len(subsystems) < 2 and not scoped_project_intelligence:
            raise ValueError(
    ...
    @staticmethod
    def _is_scoped_project_intelligence(
        metadata: dict[str, Any],
    ) -> bool:
        mode = str(metadata.get("planning_mode", "")).casefold().strip()
        source = str(metadata.get("source", "")).casefold().strip()
        return (
            mode == "project_intelligence_scoped"
            or source == "b55projectintelligence"
        )
    ...
    def _scoped_targets(
        ...
        *,
        ...
        if len(result) < 2:
            raise ValueError(
                "Tryb B55 wymaga co najmniej dwóch powiązanych plików celu."
            )
        return result[: min(self.max_targets, 6)]
    ...
    def _build_scoped_campaigns(
        ...
        *,
        ...
        baseline_stage = "scoped-baseline-validation"
        change_stage = "scoped-safe-change"
        syntax_stage = "scoped-syntax-validation"
        full_stage = "scoped-full-validation"

        return [
            {
                "campaign_id": "autonomy-01-scoped-change",
                "objective": f"{objective} — bezpieczna zmiana zakresowa.",
                "priority": "HIGH",
                "depends_on": [],
                "estimated_roi": 7.5,
                "estimated_risk": 4.0,
                "estimated_minutes": max(35, len(targets) * 18),
                "confidence": 0.86,
                "stages": [
                    {
                        "stage_id": baseline_stage,
                        "objective": "Wykonaj walidację bazową przed zmianą.",
                        "targets": list(targets),
                        "allow_same_subsystem": True,
                        "metadata": {
                            "execution_kind": "validation_only",
                            "full_suite": False,
                            "project_intelligence_scoped": True,
                        },
                    },
                    {
            ...
            {
                "campaign_id": "autonomy-02-scoped-validation",
                "objective": f"{objective} — walidacja i zabezpieczenie regresji.",
                "priority": "HIGH",
                "depends_on": ["autonomy-01-scoped-change"],
                "estimated_roi": 6.5,
                "estimated_risk": 2.0,
                "estimated_minutes": max(25, len(targets) * 10),
                "confidence": 0.92,
                "stages": [
                    {
                        "stage_id": syntax_stage,
                        "objective": "Zweryfikuj składnię i testy zakresowe.",
                        "targets": list(targets),
                        "allow_same_subsystem": True,
                        "metadata": {
                            "execution_kind": "validation_only",
                    ...
                    {
                        "stage_id": full_stage,
                        "objective": "Uruchom pełną walidację regresyjną projektu.",
                        "targets": list(targets),
                        "depends_on": [syntax_stage],
                        "allow_same_subsystem": True,
                        "metadata": {
                            "execution_kind": "validation_only",
                            "full_suite": True,
