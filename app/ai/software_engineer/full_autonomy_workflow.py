...
class FullAutonomyWorkflow:
    ...
    def resume(
        ...
        *,
        ...
        previous_policy = dict(run.get("policy", {}) or {})
        values = {
            **previous_policy,
            **dict(context or {}),
        }
        ...
        one_time_auto_approve = bool(
            values.get("_b54_one_time_auto_approve", False)
        )
        effective_auto_approve = bool(
            values.get("auto_approve", False)
        )
        persistent_auto_approve = (
            bool(previous_policy.get("auto_approve", False))
            if one_time_auto_approve
            else effective_auto_approve
        )
        run["policy"] = {
            **previous_policy,
            "auto_execute": True,
            "auto_approve": persistent_auto_approve,
            "auto_rollback": bool(
    ...
    def _fail(
        ...
        *,
        ...
        if traceback_text:
            ...
            traces = list(diagnostics.get("tracebacks", []) or [])
            traces.append({
                "status": str(status),
                "traceback": str(traceback_text)[:16000],
                "recorded_at": self._now(),
            })
            diagnostics["tracebacks"] = traces[-20:]
            diagnostics["last_traceback"] = str(traceback_text)[:16000]
        ...
        if rollback and run.get("portfolio_id"):
            try:
                ...
                diagnostics = dict(run.get("diagnostics", {}) or {})
                diagnostics["rollback_traceback"] = traceback.format_exc()[:16000]
                run["diagnostics"] = diagnostics
        run["completed_at"] = self._now()
    ...
    def _apply_learning_policy(
        ...
        if not bool(profile.get("active", False)):
            return result
        if not self._learning_profile_is_approved(profile):
            return result
    ...
    def _update_execution(
        ...
        *,
        ...
        try:
            ...
            execution["tracker_traceback"] = traceback.format_exc()[:16000]
