...
from app.ai.software_engineer.autodev_safe_preview import prepare_safe_autodev_runtime
...
class Brain:
    ...
    def project_root(self) -> str:
        stored = getattr(self, "_project_root", None)
        if stored:
            return str(stored).replace("\\", "/")
        resolved = str(resolve_project_root()).replace("\\", "/")
        self._project_root = resolved
        return resolved

    @project_root.setter
    def project_root(self, value: str | None) -> None:
        self._project_root = str(resolve_project_root(value)).replace("\\", "/")

    def __init__(
        self,
        project_root: str | None = None,
        runtime_profile: str = "owner",
    ) -> None:

        """Coordinates init for the JARVIS OS runtime."""
        selected_profile = str(runtime_profile or "owner").strip().lower()
        self.runtime_profile = (
            "client" if selected_profile == "client" else "owner"
        )
        ...
        if self.runtime_profile == "owner":
            self.background_autodev_start_result = (
                self._start_background_autodev()
            )
        else:
            self.background_autodev_start_result = {
                "success": True,
        ...
        self.autodev_safe_preview_bootstrap = None
        self.long_running_autonomy_service = None
        self.project_intelligence_service = None
        self.self_directed_development_service = None
        self.strategic_development_service = None
        self.strategic_execution_service = None
        self.strategic_portfolio_service = None
        self.strategic_policy_validation_service = None
        self.strategic_policy_evolution_service = None
        if self.runtime_profile == "owner":
            self._initialize_owner_services()
    ...
    def _start_background_autodev(self) -> dict[str, Any]:
        return self.autonomous_dev_controller.start_background(
            context={
                "project_root": self.project_root,
                "auto_approve": False,
                "auto_execute": True,
    ...
    def _initialize_owner_services(self) -> None:
        if self.strategic_policy_validation_service is not None:
            return
        self.autodev_safe_preview_bootstrap = prepare_safe_autodev_runtime(
            self.project_root
        )
        autonomy = self.software_engineer_controller
        self.long_running_autonomy_service = (
            bootstrap_long_running_autonomy(autonomy)
        )
        self.strategic_policy_validation_service = (
            bootstrap_strategic_policy_validation(autonomy)
        )
        self.strategic_policy_evolution_service = (
            self.strategic_policy_validation_service
            .strategic_policy_evolution
        )
        self.strategic_portfolio_service = (
            self.strategic_policy_evolution_service.strategic_portfolio
        )
        self.strategic_execution_service = (
            self.strategic_portfolio_service.strategic_execution
        )
        self.strategic_development_service = (
            self.strategic_execution_service.strategic_development
        )
        self.self_directed_development_service = (
            self.strategic_execution_service.self_directed
        )
        self.project_intelligence_service = (
            self.strategic_execution_service.project_intelligence
        )

    def enable_owner_runtime(self) -> dict[str, Any]:
        """Load owner-only autonomy only after the protected mode is opened."""
        self.runtime_profile = "owner"
        self._initialize_owner_services()
        ...
        return {
            ...
            "background": dict(self.background_autodev_start_result),
        }
