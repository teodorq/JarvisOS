...
function newRequestId(){const bytes=new Uint8Array(16);crypto.getRandomValues(bytes);return Array.from(bytes,value=>value.toString(16).padStart(2,"0")).join("")}
async function requestIdFor(target,value){const input=new TextEncoder().encode(target+"\0"+value),digest=await crypto.subtle.digest("SHA-256",input),signature=Array.from(new Uint8Array(digest),byte=>byte.toString(16).padStart(2,"0")).join(""),key="jarvisPendingRequest";let stored={};try{stored=JSON.parse(sessionStorage.getItem(key)||"{}")}catch(_error){}if(!/^[a-f0-9]{32}$/.test(stored.id||"")||stored.signature!==signature){stored={id:newRequestId(),signature};sessionStorage.setItem(key,JSON.stringify(stored))}return stored.id}
function clearPendingRequest(){sessionStorage.removeItem("jarvisPendingRequest")}
